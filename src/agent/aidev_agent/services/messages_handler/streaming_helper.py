                ...
                try:
                    ...
                    if enable_heartbeat_check and time_since_last > HEARTBEAT_TIMEOUT:
                        ...
                        if producer_finished:
                            ...
                            exit_reason = "completed"
                            return exit_reason
                        # producer 还活着：长间隔工具调用（如 LLM 思考、工具执行）产出间隔
                        # 可能超过 HEARTBEAT_TIMEOUT（15s），此时不应误判 starved。
                        # 延长到 HEARTBEAT_TIMEOUT * 4（60s），仅当 producer 长时间无产出才判定崩溃。
                        if time_since_last <= HEARTBEAT_TIMEOUT * 4:
                            logger.info(
                                "[RabbitMQ] producer 仍存活，延长心跳等待 thread_id=%s "
                                "time_since_last=%.1fs threshold=%.1fs",
                                self.thread_id,
                                time_since_last,
                                HEARTBEAT_TIMEOUT * 4,
                            )
                        ...
                        exit_reason = "starved"
                        raise RuntimeError(
                            f"生产者心跳超时：超过 {HEARTBEAT_TIMEOUT * 4}s 未收到任何消息，生产者可能已崩溃"
                        )
                    continue
