...
class BKDjangoSaver(BaseCheckpointSaver[str]):
    ...
    def __init__(
        ...
        *,
        ...
        # task_path 是 langgraph 后加字段，老业务方 writes_model 可能没有
        self._writes_has_task_path = any(f.name == "task_path" for f in self.writes_model._meta.local_fields)
    ...
    def put_writes(
        ...
        if self._writes_has_task_path:
            shared_fields["task_path"] = task_path or ""
        writes_objects = []
        ...
        def save_writes() -> None:
            if all(w[0] in WRITES_IDX_MAP for w in writes):
                # bulk_upsert 只 INSERT update ∪ unique 列；task_path 无 SQL default，漏列会触发 MySQL 1364
