...
import pytest
from aidev_bkplugin.models import Checkpoint
...
class WriteWithTaskPathForTest(models.Model):
    """带 task_path 的 writes_model：复现 MySQL 严格模式 1364。"""
...
def _make_put_writes_saver(mocker, writes_model):
    """走真实 __init__ 探测字段，再替换 serde 以便断言 put_writes。"""
    instance = BKDjangoSaver(Checkpoint, writes_model)
    instance.serde = mocker.Mock()
...
def test_init_detects_task_path_on_new_and_legacy_write_model():
    assert BKDjangoSaver(Checkpoint, WriteWithTaskPathForTest)._writes_has_task_path is True
    assert BKDjangoSaver(Checkpoint, WriteForTest)._writes_has_task_path is False


def test_put_writes_new_model_appends_task_path_to_update_fields_and_write_obj(mocker):
    saver = _make_put_writes_saver(mocker, WriteWithTaskPathForTest)
    ...
    assert "task_path" in call.kwargs["update_fields"]
    assert call.kwargs["unique_fields"] == ["thread_id", "checkpoint_ns", "checkpoint_id", "task_id", "idx"]
    assert call.args[1][0].task_path == "('__pregel_pull','agent')"


def test_put_writes_old_model_skips_task_path(mocker):
    saver = _make_put_writes_saver(mocker, WriteForTest)
    bulk_upsert_mock = _patch_write_pipeline(mocker)
    ...
    call = bulk_upsert_mock.call_args
    assert "task_path" not in call.kwargs["update_fields"]
    assert not hasattr(call.args[1][0], "task_path")
...
def test_put_writes_new_model_falsy_task_path_falls_back_to_empty_string(mocker):
    ...
    for call in bulk_upsert_mock.call_args_list:
        assert call.args[1][0].task_path == ""
